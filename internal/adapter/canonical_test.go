...
func TestOwnershipSurvivesPathSpelling(t *testing.T) {
	...
	if _, _, _, _, err := LinkSkills(skills, vaultSkillsDir, dst, false); err != nil {
		t.Fatal(err)
	}
	...
	_, _, _, blocked, err := LinkSkills(realSkills, realVaultSkillsDir, realDst, false)
	if err != nil {
		t.Fatal(err)
	}
	if len(blocked) != 0 {
...
func TestPruneRemovesStaleLinkThroughDanglingSpelling(t *testing.T) {
	...
	if _, _, _, _, err := LinkSkills(skills[1:], vaultSkillsDir, dst, false); err != nil {
		t.Fatal(err)
	}
