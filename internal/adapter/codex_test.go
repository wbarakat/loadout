...
func TestCodexApplyAndCheck(t *testing.T) {
	v := testVault(t)
	home := t.TempDir()
...
func TestCodexApplyDryRunWritesNothingAndReportsStatus(t *testing.T) {
	v := testVault(t)
	home := t.TempDir()
	cfg := vault.AdapterConfig{
		Enabled:    true,
...
func TestCodexApplyProjectsMemoryDespiteBlockedSkill(t *testing.T) {
	v := testVault(t)
	home := t.TempDir()
	cfg := vault.AdapterConfig{
		Enabled:    true,
...
func TestCodexEnabledInRegistry(t *testing.T) {
	m := vault.DefaultManifest()
	cfg, ok := m.Adapters["codex"]
	if !ok {
		t.Fatal("codex must be in the default manifest")
	}
	if cfg.Enabled {
		t.Fatal("codex must be disabled by default")
	}
	if cfg.SkillsDir != "~/.codex/skills" {
		t.Fatalf("bad skills dir: %q", cfg.SkillsDir)
	}
	if cfg.MemoryFile != "~/.codex/AGENTS.md" {
		t.Fatalf("bad memory file: %q", cfg.MemoryFile)
	}

	// Test that Enabled returns codex when enabled in manifest.
	v := testVault(t)
	v.Manifest.Adapters["codex"] = vault.AdapterConfig{
		Enabled:    true,
		SkillsDir:  "~/.codex/skills",
		MemoryFile: "~/.codex/AGENTS.md",
	}
	...
	if len(names) != 3 || names[0] != "claude-code" || names[1] != "pi" || names[2] != "codex" {
